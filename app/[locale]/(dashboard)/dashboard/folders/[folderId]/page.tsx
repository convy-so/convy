...
function FolderDetailContent() {
  ...
  // Calculate stats
  // Calculate stats
  const totalStarted = folder.surveys.reduce((acc, s) => acc + (s.currentParticipants || 0), 0);
  const totalCompleted = folder.surveys.reduce((acc, s) => acc + (s.completedCount || 0), 0);

  const stats = {
    totalSurveys: folder.surveys.length,
    totalResponses: totalStarted,
    avgCompletion: totalStarted > 0 ? Math.round((totalCompleted / totalStarted) * 100) : 0,
    activeSurveys: folder.surveys.filter(s => s.status === 'active').length,
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${folder.color || 'from-gray-500 to-gray-600'} flex items-center justify-center`}>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <span className="text-sm text-gray-500">{folder.surveys.length} surveys</span>
            </div>
                              ...
                              <div className="absolute right-0 top-full mt-1 w-40 bg-white rounded-xl border border-gray-200 shadow-xl z-50 py-1">
                                ...
                                {folder.canOrganizeSurveys ? (
                                  <>
                                    ...
                                    <button
                                      onClick={() => {
                                        removeSurveyMutation.mutate({ folderId, surveyId: survey.id });
                                        setShowMenuFor(null);
                                      }}
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-gray-900">{new Date(folder.createdAt).toLocaleDateString()}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">Updated</span>
                <span className="text-sm text-gray-900">{folder.updatedAt ? new Date(folder.updatedAt).toLocaleDateString() : '-'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">Color</span>
                <div className={`w-4 h-4 rounded-full bg-gradient-to-br ${folder.color || 'bg-gray-500'}`} />
              </div>
            </div>
          </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
            <div className="p-6 text-center">
              ...
              <p className="text-gray-500">
                Are you sure you want to delete <span className="font-semibold text-gray-900">&quot;{folder.name}&quot;</span>? This action cannot be undone and will remove all survey associations.
              </p>
            </div>
