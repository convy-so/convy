...
import { buildBudgetedContextBundle } from "@/features/tutoring/server/context-engineering";
  ...
  return {
    ...
    framework: {
      ...
      capabilityGuidance: {
        search_image: {
          enabled: true,
          policy: "Use images only when they directly support diagnosis or checking work.",
          maxUsesPerTurn: 2,
        },
        search_video: {
          enabled: false,
        ...
        administer_quiz: {
          enabled: true,
          policy: "Quiz after the student has had one supported attempt.",
        },
        grade_student_work: {
          enabled: true,
          policy: "Grade only a submitted quiz answer.",
        },
        finish_session: {
          policy: "Finish only with concrete completion evidence and a next step.",
        },
      },
      fewShotExamples: [
...
function run() {
  ...
  assert.ok(
    promptRuntime.dynamicSystemPrompt.includes(
      "Capability policy (authoritative for tool use and overrides any tool references in the framework markdown)",
    ),
    "expected the prompt runtime to state that capability policy is authoritative for tool use",
  );
  assert.ok(
    promptRuntime.dynamicSystemPrompt.includes(
      "search_video (Educational videos): disabled.",
    ),
