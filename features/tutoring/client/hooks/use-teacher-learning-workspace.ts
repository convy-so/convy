...
import type { ClassroomStudent, PendingInvitation, Topic } from "@/features/tutoring/public-client";
import { queryKeys } from "@/shared/http/query-keys";
import { getFriendlyActionError } from "@/shared/http/friendly-action-error";
import type { getTeacherLearningWorkspaceInitialData } from "@/shared/http/page-data";
...
export function useTeacherLearningWorkspace(
  ...
  const topicsQuery = useQuery<
    Awaited<ReturnType<typeof fetchClassroomTopics>>
  >({
    queryKey: selectedAccessibleClassroomId
    ...
    initialData:
      ...
      selectedAccessibleClassroomId ===
        (initialData.initialClassrooms.data[0]?.id ?? null)
        ? (initialData.initialTopics as Awaited<
            ReturnType<typeof fetchClassroomTopics>
          >)
        : undefined,
    staleTime: 30_000,
    retry: retryTransientLearningApiFailure,
