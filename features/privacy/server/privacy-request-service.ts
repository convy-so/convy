...
export async function createPrivacyRequest(input: {
  ...
  const [request] = await getDb()
    ...
    .values({
      ...
      status: PRIVACY_DEFAULTS.statusPending,
  ...
  return requireValue(request, "Failed to create privacy request.");
}
...
export async function createDeletionJob(input: {
  ...
  return requireValue(job, "Failed to create deletion job.");
}
...
export async function markDeletionJobStatus(input: {
  ...
  await getDb()
    ...
    .set({
      ...
      startedAt:
        input.status === PRIVACY_DEFAULTS.statusInProgress
          ? new Date()
          : undefined,
      completedAt:
        input.status === PRIVACY_DEFAULTS.statusCompleted ||
        input.status === PRIVACY_DEFAULTS.statusFailed
          ? new Date()
          : undefined,
      updatedAt: new Date(),
    })
