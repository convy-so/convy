...
} from "@/features/surveys/voice/voice-agent-provider";
...
export abstract class BaseVoiceAgentHandler {
  ...
  private setupBrowserMessageHandlers(): void {
    this.ws.on("message", (data: Buffer, isBinary: boolean) => {
      void this.handleBrowserMessage(data, isBinary);
    });
  }
    ...
    isBinary: boolean,
      ...
      if (!isBinary) {
        try {
          ...
          const message = parseJsonValue(strData);
