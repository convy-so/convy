...
import { sampleConversations, surveys, voiceSessions } from "@/shared/db/schema";
...
import { buildSampleVoiceFunctions } from "@/features/surveys/server/education/agent-tools";
import {
...
import type { AuthenticatedConnection } from "@/shared/realtime/middleware/auth";
...
import { isSupportedVoiceLocale, normalizeVoiceLocale } from "@/features/surveys/voice/voice-locales";
...
const SAMPLE_SURVEY_VOICE_DELAY_MS = {
  finishFunction: 500,
} as const;
...
export class SampleSurveyVoiceHandler extends BaseVoiceAgentHandler {
  ...
  private getRequiredUserId(): string {
    if (!this.userId) {
      throw new Error("Sample survey voice requires an authenticated user");
    }
  ...
  async initialize(): Promise<void> {
    try {
      ...
      await getDb().insert(voiceSessions).values({
        ...
        surveyId: survey.id,
        userId,
        sessionType: VOICE_SESSION_TYPE.SAMPLE_CONVERSATION,
      ...
      if (!sampleConversation) {
        ...
        sampleConversation = requireValue(
          created,
          `Failed to create sample conversation ${this.state.conversationNumber} for survey ${survey.id}`,
        );
        await getDb()
          .update(surveys)
          .set({
      ...
      if (!sessionRow) {
        sessionRow = await ensureSession({
          surveyId: survey.id,
          sessionType: SURVEY_SESSION_TYPE.SAMPLE,
      ...
      const readySessionRow = requireValue(
        sessionRow,
        `Failed to resolve sample session for conversation ${sampleConversation.id}`,
      );

      this.state.sessionId = readySessionRow.id;
      this.state.sessionState = readySessionRow.sessionState;
  ...
  protected getInitialUserInput(): string | null {
    ...
    const lastMessage = requireValue(
      ...
      "Sample survey voice expected a last message when messages are present",
    );
...
- Keep the exchange realistic and participant-centered.`;
    ...
    return buildVoiceAgentSettings({
      language: this.state.language,
      tone: normalizeSurveyTone(this.state.survey.tone),
      systemPrompt,
...
- Keep the exchange realistic and participant-centered.`;
    ...
    switch (event.function_name) {
      case "finishSurvey": {
        ...
        setTimeout(() => {
          ...
          void this.cleanup();
        }, SAMPLE_SURVEY_VOICE_DELAY_MS.finishFunction);
        return;
      }
  ...
  protected async cleanup(): Promise<void> {
    ...
    getDb()
      ...
      .set({ status: VOICE_SESSION_STATUS.COMPLETED, endedAt: new Date() })
