...
const CHAT_WITH_DATA_SUGGESTION = {
    POSITIVE_OUTCOMES: "How many students reported positive outcomes?",
    COMMON_BARRIERS: "What barriers appear most often?",
    SNAPSHOT_COMPARISON: "Compare the latest snapshot with the previous one.",
    TOP_FINDING_EVIDENCE: "Show the strongest evidence behind the top finding.",
} as const;
...
function parseChatMessages(values: readonly unknown[]): ChatMessage[] {
    return toUIMessages(toPersistedUIChatMessages(values)).map((message) => ({
        id: message.id,
        role: message.role,
        parts: message.parts,
    }));
...
function parseChatSessionMutationResponse(value: unknown): { session: ChatSession } | null {
...
function parseChatSessionDetailResponse(value: unknown): { session: ChatSessionDetail } | null {
    if (!isRecord(value) || !isRecord(value.session)) {
        return null;
    }
...
export function ChatWithData({ surveyId }: ChatWithDataProps) {
    ...
    const { messages, setMessages, sendMessage, status } = useChat<ChatMessage>({
        ...
        onFinish: ({ message, messages: finishedMessages }) => {
            void saveSession([...finishedMessages, message]);
        },
    });
    ...
    const saveSession = async (currentMessages: ChatMessage[]) => {
        try {
            ...
            if (res.ok) {
                const parsedResponse = parseChatSessionMutationResponse(await res.json());
                if (!parsedResponse) {
    ...
    const loadSession = async (sessionId: string) => {
        try {
            ...
            if (res.ok) {
                const parsedResponse = parseChatSessionDetailResponse(await res.json());
                if (!parsedResponse) {
                    throw new Error("Invalid chat session detail response");
                }
        ...
        try {
            ...
            recorder.onstop = async () => {
                ...
                try {
                    ...
                    const data = parseTranscriptionResponse(await response.json());
                    if (!response.ok) {
            ...
            <div className="p-6 border-t border-gray-100 flex-shrink-0">
                ...
                <div className="max-w-4xl mx-auto flex gap-4 mt-6 overflow-x-auto pb-2 px-2 no-scrollbar">
                    <SuggestionPill
                        ...
                        onClick={() => setInput(CHAT_WITH_DATA_SUGGESTION.COMMON_BARRIERS)}
                    />
                    <SuggestionPill
                        icon={Table}
                        label="Pivot data"
                        onClick={() => setInput(CHAT_WITH_DATA_SUGGESTION.POSITIVE_OUTCOMES)}
                    />
                    <SuggestionPill
                        icon={BarChart2}
                        label="Visualize metrics"
                        onClick={() => setInput(CHAT_WITH_DATA_SUGGESTION.SNAPSHOT_COMPARISON)}
                    />
                    <SuggestionPill
                        icon={Sparkles}
                        label="AI Insights"
                        onClick={() => setInput(CHAT_WITH_DATA_SUGGESTION.TOP_FINDING_EVIDENCE)}
                    />
                </div>
            </div>
        </div>
