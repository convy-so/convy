...
import { resolveTeacherClassroomAccess } from "@/features/tutoring/public-server";
import { getSurveyPermissionContext, hasSurveyPermission } from "@/features/surveys/public-server";
...
import type { createSurveySchema } from "@/features/surveys/server/validation/survey-schemas";
...
export async function createSurveyForUser(params: {
  ...
  const language = resolveUiLocaleForContentCreation({
  ...
  await getDb().transaction(async (tx) => {
    const [inserted] = await tx
      ...
      .values({
        ...
        deliveryMode,
        title:
          deliveryMode === SURVEY_DELIVERY_MODE.CLASSROOM_ASSIGNED
    ...
    await tx.insert(surveyCreationConversations).values({
      ...
      surveyId,
      messages: [],
      status: CREATION_CONVERSATION_STATUS.IN_PROGRESS,
      collectedInfo: {},
      extractedData: {},
      createdAt: now,
      updatedAt: now,
