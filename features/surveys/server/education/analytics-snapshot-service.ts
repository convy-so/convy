...
import { getPromptSpec } from "@/shared/ai/prompt-specs";
...
import { replaceEmbeddedSource } from "@/shared/retrieval/indexer";
...
function aggregateCoverage(insights: ConversationInsight[], nodeIds: string[]) {
  ...
  for (const insight of insights) {
    for (const id of nodeIds) {
      totals[id] =
        (totals[id] ?? 0) + Math.max(0, Math.min(1, insight.nodeCoverage[id] ?? 0));
    }
  }
  const divisor = insights.length || 1;
  for (const id of nodeIds) {
    totals[id] = (totals[id] ?? 0) / divisor;
  }
  const overall = nodeIds.length
    ? nodeIds.reduce((sum, id) => sum + (totals[id] ?? 0), 0) / nodeIds.length
    : 0;
