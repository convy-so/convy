...
const DEFAULT_FEEDBACK_KIND: FeedbackKind = "suggestion";
const DEFAULT_SOURCE_AREA: FeedbackSourceArea = "classroom";
...
const feedbackKindOptions: Array<{ label: string; value: FeedbackKind }> = [
  { label: "Suggestion", value: "suggestion" },
  { label: "Complaint", value: "complaint" },
];

const selectableSourceAreas: FeedbackSourceArea[] = [
  "survey",
  "tutoring",
  "classroom",
  "expert_ops",
...
export function FeedbackForm({
  ...
  const [kind, setKind] = useState<FeedbackKind>(DEFAULT_FEEDBACK_KIND);
  const [sourceArea, setSourceArea] = useState<FeedbackSourceArea>(DEFAULT_SOURCE_AREA);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          getResponseErrorMessage(data) ?? "Failed to submit feedback",
        );
      }

      toast.success("Your feedback has been submitted.");
      setKind(DEFAULT_FEEDBACK_KIND);
      setSourceArea(DEFAULT_SOURCE_AREA);
      setSubject("");
      setMessage("");
    } catch (error) {
      toast.error(
      ...
      >
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          <CustomSelect
            label="Area"
            value={sourceArea}
            options={selectableSourceAreas.map((key) => ({
