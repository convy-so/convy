...
export const VOICE_ERRORS = {
  ...
  AUDIO_TOO_LARGE: {
    ...
    description: "Audio chunk exceeds size limit",
  ...
  SESSION_EXPIRED: {
    ...
    description: "Voice session expired",
...
export function mapBrowserErrorToVoiceError(error: Error): VoiceError {
  ...
  // Microphone permission errors
  if (description.includes("permission") || description.includes("denied")) {
    return createVoiceError("MICROPHONE_PERMISSION_DENIED", error.message);
  }

  if (description.includes("not found") || description.includes("no device")) {
    return createVoiceError("MICROPHONE_NOT_FOUND", error.message);
  }

  if (description.includes("in use") || description.includes("busy")) {
    return createVoiceError("MICROPHONE_IN_USE", error.message);
  }

  // Network errors
  if (description.includes("network") || description.includes("timeout")) {
    return createVoiceError("NETWORK_TIMEOUT", error.message);
  }

  // WebSocket errors
  if (description.includes("websocket") || description.includes("connection")) {
