...
/** Valid domain IDs: 1=CX, 2=Market Research, 3=Workforce, 5=Education, 6=Civic, 7=Scientific, 9=Demographic, 10=Infrastructure */
const VALID_DOMAIN_IDS = new Set([1, 2, 3, 5, 6, 7, 9, 10]);
...
export function validateExtractedData(data: ExtractedData): ValidationIssue[] {
  ...
  if (data.domainId) {
    if (!VALID_DOMAIN_IDS.has(data.domainId)) {
      issues.push({
        field: "domainId",
  ...
  if (data.objective && data.scope) {
    if (
      data.scope.breadthVsDepth === "deep" &&
      (!data.objective.goal || data.objective.goal.length < 20)
    ) {
      issues.push({
      ...
      if (m.type === "video" || m.type === "audio") {
        if (data.constraints?.timeLimit && data.constraints.timeLimit < 5) {
          issues.push({
            field: `media[${idx}]`,
            issue:
              "Video/Audio media included but survey time limit is very short (< 5 mins).",
            severity: "warning",
          });
        }
      }
