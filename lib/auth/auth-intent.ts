...
const AUTH_INTENT_COOKIE = "convy_auth_intent";
const AUTH_INTENT_TTL_SECONDS = 60 * 15;

export const authIntentKindSchema = z.enum([
  "direct-signup",
  "invite-signup",
  "invite-signin",
  "plain-signin",
...
export const authIntentRoleSchema = z.enum(["student", "teacher"]);
...
export async function readAuthIntentCookie(): Promise<AuthIntent | null> {
  ...
  const value = cookieStore.get(AUTH_INTENT_COOKIE)?.value;
...
export async function readAuthIntentFromRequestHeaders(
  ...
  const entry = parts.find((part) => part.startsWith(`${AUTH_INTENT_COOKIE}=`));
  if (!entry) return null;

  return decodeIntentCookie(entry.slice(AUTH_INTENT_COOKIE.length + 1));
}
