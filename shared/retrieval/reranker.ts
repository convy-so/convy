      ...
      for (const item of results) {
        ...
        if (index !== undefined && index >= 0 && index < candidates.length) {
          const candidate = requireValue(
            candidates[index],
            `Missing rerank candidate at index ${index}.`,
          );
          rankedResults.push({
            ...candidate,
      ...
      if (rankedResults.length < topK) {
        for (let i = 0; i < candidates.length; i++) {
          if (!seenIndices.has(i)) {
            rankedResults.push(
              requireValue(candidates[i], `Missing rerank candidate at index ${i}.`),
            );
  ...
  try {
    ...
    for (const index of object.rankedIndices) {
      if (index >= 0 && index < candidates.length && !seenIndices.has(index)) {
        rankedResults.push(
          requireValue(candidates[index], `Missing rerank candidate at index ${index}.`),
        );
        seenIndices.add(index);
      }
      if (rankedResults.length >= topK) break;
