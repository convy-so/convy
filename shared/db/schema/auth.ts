...
import {
  AUTH_DEFAULTS,
  EXPERT_INVITATION_STATUS_VALUES,
...
const users = pgTable(
  "users",
  {
    ...
    uiLocale: text("ui_locale").default(AUTH_DEFAULTS.locale),
    preferredLanguage: text("preferred_language").default(AUTH_DEFAULTS.locale),
  },
  (table) => [
...
const expertInvitationStatusEnum = pgEnum(
  "expert_invitation_status",
  EXPERT_INVITATION_STATUS_VALUES,
);
...
const expertInvitations = pgTable(
  "expert_invitations",
  {
    ...
    locale: text("locale").default(AUTH_DEFAULTS.locale).notNull(),
    status: expertInvitationStatusEnum("status")
      .default(EXPERT_INVITATION_STATUS_VALUES[0])
      .notNull(),
    expiresAt: timestamp("expires_at", {
      withTimezone: true,
      mode: "date",
  ...
  (table) => [
    ...
    uniqueIndex("expert_invitations_pending_email_unique")
      ...
      .where(sql`${table.status} = ${EXPERT_INVITATION_STATUS_VALUES[0]}`),
    check(
