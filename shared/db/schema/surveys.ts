...
import type {
  ...
  SampleFeedbackPatch,
} from "@/features/surveys/server/education/sample-feedback";
import type {
  ResearchBriefPatch,
  RefinementMessage,
  RefinementProposal,
} from "@/features/surveys/server/education/refinement-schemas";
import {
  surveyStatusEnum,
  toneEnum,
  languageEnum,
...
export const MAX_CONVERSATION_DURATION_MINUTES =
  SURVEY_LIMITS.maxConversationDurationMinutes;

export type SurveyHypotheses = {
  assumptions: string[];
};

import { SurveyMedia } from "@/features/surveys/server/education/brief-media";
...
const surveys = pgTable(
  "surveys",
  {
    ...
    deliveryMode: text("delivery_mode")
      .default(SURVEY_DEFAULTS.deliveryMode)
      .notNull(),
    title: text("title").notNull(),
    description: text("description"),
    coreObjective: text("core_objective"),
    tone: toneEnum("tone").default(SURVEY_DEFAULTS.tone),
    ...
    status: surveyStatusEnum("status").default(SURVEY_DEFAULTS.status).notNull(),
    shareableLink: text("shareable_link").unique(),
    customSlug: text("custom_slug").unique(),
    participantLimit: integer("participant_limit")
      .default(SURVEY_DEFAULTS.participantLimit)
      .notNull(),
    currentParticipants: integer("current_participants")
      .default(SURVEY_DEFAULTS.currentParticipants)
      .notNull(),
    sampleConversationCount: integer("sample_conversation_count")
      .default(SURVEY_DEFAULTS.sampleConversationCount)
      .notNull(),
    programId: text("program_id"),
    confirmed: boolean("confirmed").default(SURVEY_DEFAULTS.confirmed).notNull(),
    language: languageEnum("language").default(SURVEY_DEFAULTS.language).notNull(),
    isVoice: boolean("is_voice").default(SURVEY_DEFAULTS.isVoice).notNull(),
  },
  (table) => [
...
const surveyCreationConversations = pgTable(
  "survey_creation_conversations",
  {
    ...
    messages: jsonb("messages")
      ...
      .notNull()
      .default([]),
    durationMs: integer("duration_ms").default(SURVEY_DEFAULTS.durationMs),
    activeDurationMs: integer("active_duration_ms").default(
      SURVEY_DEFAULTS.activeDurationMs,
    ),
    status: creationConversationStatusEnum("status")
      .default(CREATION_CONVERSATION_STATUS.IN_PROGRESS)
      .notNull(),
...
const surveyConversations = pgTable(
  "survey_conversations",
  {
    ...
    completed: boolean("completed").default(SURVEY_DEFAULTS.confirmed).notNull(),
    originalLanguage: text("original_language").default(SURVEY_DEFAULTS.language),
...
const surveyBriefs = pgTable(
  "survey_briefs",
  {
    ...
    surveyId: text("survey_id")
      ...
      .references(() => surveys.id, { onDelete: "cascade" })
      .unique(),
    version: integer("version").default(SURVEY_DEFAULTS.initialVersion).notNull(),
...
const surveySessions = pgTable(
  "survey_sessions",
  {
    ...
    sessionStatus: text("session_status")
      .default(SURVEY_SESSION_STATUS.ACTIVE)
      .notNull(),
    sourceConversationId: text("source_conversation_id"),
    language: text("language").default(SURVEY_DEFAULTS.language).notNull(),
...
const surveyEvidence = pgTable(
  "survey_evidence",
  {
    ...
    reliability: integer("reliability")
      .default(SURVEY_DEFAULTS.evidenceReliabilityPercent)
      .notNull(),
...
export const participantFeedback = pgTable(
  "participant_feedback",
  {
    ...
    rating: integer("rating"), // SURVEY_DEFAULTS.participantFeedbackMinRating-SURVEY_DEFAULTS.participantFeedbackMaxRating
...
const analyticsChatSessions = pgTable(
  "analytics_chat_sessions",
  {
    ...
    userId: text("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    title: text("title").notNull().default(SURVEY_DEFAULTS.analyticsChatTitle),
    messages: jsonb("messages")
    ...
    lastProcessedResponseCount: integer("last_processed_response_count")
      .notNull()
      .default(SURVEY_DEFAULTS.lastProcessedResponseCount),
  },
  (table) => [
