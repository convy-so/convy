...
import { AI_DEFAULTS, EXPERT_GUIDANCE_STATUS_VALUES } from "@/shared/ai/constants";
...
export const expertGuidancePacks = pgTable(
  "expert_guidance_packs",
  {
    ...
    status: text("status").default(AI_DEFAULTS.draftStatus).notNull(),
    name: text("name").notNull(),
    description: text("description"),
    targetScope: text("target_scope").default(AI_DEFAULTS.targetScopeGlobal).notNull(),
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.status} in (${sql.join(EXPERT_GUIDANCE_STATUS_VALUES.map((value) => sql`${value}`), sql`, `)})`,
    ),
  ],
);
...
export const expertGuidanceVersions = pgTable(
  "expert_guidance_versions",
  {
    ...
    version: integer("version").default(AI_DEFAULTS.initialVersion).notNull(),
    status: text("status").default(AI_DEFAULTS.draftStatus).notNull(),
...
export const fewShotExamples = pgTable(
  "few_shot_examples",
  {
    ...
    retrievalContent: text("retrieval_content")
      .notNull()
      .default(AI_DEFAULTS.retrievalContent),
    content: jsonb("content").$type<Record<string, unknown>>().notNull().default({}),
    isActive: boolean("is_active").default(true).notNull(),
    ownedByRole: text("owned_by_role").default(AI_DEFAULTS.ownedByRoleExpert).notNull(),
    createdByUserId: text("created_by_user_id").references(() => users.id, {
      onDelete: "set null",
    }),
    metadata: jsonb("metadata").$type<Record<string, unknown>>().default({}),
