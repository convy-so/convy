...
const DEFAULT_PRICING_MODEL = "gemini-3.1-flash-lite";
const DEFAULT_MODEL_PRICING = requireValue(
  PRICING[DEFAULT_PRICING_MODEL],
  `Missing default pricing model ${DEFAULT_PRICING_MODEL}`,
);

export function calculateCost(input: UsageLogInput): string {
  if (input.type === "llm_text" && input.modelName) {
    const pricing = PRICING[input.modelName] ?? DEFAULT_MODEL_PRICING;
