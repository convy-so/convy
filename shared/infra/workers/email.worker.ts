...
import type { EmailJobData } from "@/shared/infra/queue";
import { getRedisClient } from "@/shared/infra/redis";
import { VerificationEmail } from "@/features/auth/email/verification";
import { PasswordResetEmail } from "@/features/auth/email/password-reset";
import { ExpertInvitationVerificationEmail } from "@/features/auth/email/expert-invitation-verification";
import { ExpertPasswordSetupEmail } from "@/features/auth/email/expert-password-setup";
import { SecondaryVerificationEmail } from "@/features/auth/email/secondary-verification";
import { SurveyDeletedEmail } from "@/features/surveys/email/survey-deleted";
import { StudentInvitationEmail } from "@/features/tutoring/email/student-invitation";
...
function getErrorName(value: unknown): string | null {
...
function getLocalizedEmailCopy(locale: ReturnType<typeof getMetadataLocale>) {
  ...
  > = {
    ...
    fr: {
      ...
      studentInvitationSubject: (classroomName: string) =>
        `Vous avez Ã©tÃ© invitÃ© Ã  rejoindre ${classroomName}`,
    },
    de: {
  ...
  async (job: Job<EmailJobData>) => {
    ...
    if (result.error) {
      ...
      console.error(`[email-worker] Resend API returned an error`, {
        ...
        errorName: getErrorName(result.error),
        errorMessage: result.error.message,
      });
      throw new Error(
        `Resend rejected the email: [${getErrorName(result.error) ?? "unknown"}] ${result.error.message}`,
      );
    }
...
emailWorker.on("completed", (job, result) => {
  console.log(`[email-worker] Job completed`, {
    jobId: job.id,
    type: (result as Record<string, unknown> | undefined)?.type,
    to: (result as Record<string, unknown> | undefined)?.email,
    resendMessageId: (result as Record<string, unknown> | undefined)?.messageId,
  });
});
