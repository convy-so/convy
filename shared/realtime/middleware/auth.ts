...
import { sessions, users, surveys } from "@/shared/db/schema";
...
import { resolveTrustedNodeClientIp } from "@/shared/security/client-ip";
import { getPlatformRole, type PlatformRole, AuthError as DalAuthError } from "@/features/auth/public-server";
import { assertPermission } from "@/features/auth/public-server";
  ...
  const respondentAccess = await resolveRespondentAccess({
    ...
    userAgent:
      Array.isArray(request.headers["user-agent"])
        ? (request.headers["user-agent"] as string[])[0] ?? null
